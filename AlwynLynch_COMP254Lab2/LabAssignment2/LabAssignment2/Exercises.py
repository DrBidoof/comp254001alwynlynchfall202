...
def example3(arr: List[int]) -> int:
    ...
    for j in range(n):            # 1 + n +n = 2n +1
        for k in range(j + 1):    # n² + 2n used AP sum formula
            total += arr[j]       # 1.5n^2 + 1.5n used AP sum formula
    return total                  # 1
                                  # 2.5n^2 + 3.5n + 5
                                  # O(n^2)
...
def example5(first: List[int], second: List[int]) -> int:
    ...
    for i in range(n):            # 2n+1
        total = 0                 # n
        for j in range(n):        # 2n+1
            for k in range(j + 1):# n^2+2n used AP sum formula
                total += first[k] # 1.5n^2 + 1.5n  used AP sum formula
        if second[i] == total:    # 2
            count += 1            # 1
    return count                  # 1
                                  #let i be the number of times the out loop runs
                                  # then i = n 
                                  # so total ops = i(2.5n^2 + 5.5n + 5) + 5
